...
布局偏好：主控 Pi 保持在当前主控 pane；如果 Herdr 不支持左侧固定主控、右侧独立 tab 的左右分屏，不要把第一个后台 agent 放到主控所在 tab 的右侧 pane。所有后台 agent 都在当前主控 workspace 中创建独立 tab，并使用 `--no-focus`，由用户通过鼠标点击 tab 切换；主控不得主动切焦点。
