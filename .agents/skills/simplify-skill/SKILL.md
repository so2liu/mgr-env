...
它围绕一个既有的目标文件工作，产出四环：证据审计 → 精简方案评审 → 精简编辑 → 独立验收。其中“精简方案评审”是用户决策环——在动笔前把要砍/改的方案交给用户拍板，而不是单据面证据自行决定。证据审计、精简编辑、独立验收由子会话完成；精简方案评审由根智能体把审计结果整理成方案，交由用户确认。
...
5. 精简方案评审（用户决策环）：在证据审计之后、精简编辑之前，把审计结论整理成一份“砍哪、改哪、为什么”的精简方案，交由用户拍板。用户确认后才进入精简编辑；用户要求调整则回到方案评审。未获用户确认，不得进入精简编辑。
6. 前一阶段只传结构化事实，不传自由形式思考记录。建议字段为 `target`、`classification`、`owner`、`evidence`、`risk`、`action`。
7. 问题排查初始只有表头，无数据行。之后只追加真实发生、可复现的违反场景，不预填猜想问题。
8. Mermaid 是可选但推荐的：流程性 skill 通常需要它，知识性 skill 可以不画；画时须表达可观察的输入、决策、停点、处理和输出，不是装饰。
...
3. **精简方案评审**：根智能体把证据审计结论整理成一份精简方案——“砍哪、改哪、为什么、每处的证据”，交由用户拍板。用户确认后进入精简编辑；用户要求调整则回到本步迭代。未获用户确认，不得进入精简编辑。
4. **精简编辑**：委派子智能体读取 `references/prose-audit.md` 和已确认的精简方案，修改目标文件；不得扩大审计范围，不得写入讨论过程。产出编辑 diff。
5. **独立验收**：委派子智能体读取 `references/acceptance.md`，检查 diff、引用、遗漏的真实边界和仓库门禁；验收者不得复用编辑者。产出验收结论。
6. **决定接受或返工**：根智能体根据验收结果决定接受或返工。根不复述 reference 内容，不把子智能体的分析全文写入目标或最终报告。
7. **交付评审材料**：输出一份规则—owner—证据表（不写回目标文件），说明保留了哪些、迁移/压缩/删除了哪些及证据。
